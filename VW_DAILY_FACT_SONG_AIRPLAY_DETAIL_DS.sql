...
SELECT
  e.SONG_ID,
  e.MARKET_ID,          -- VARCHAR in this view
  e.MARKET_NAME,
  e.COUNTRY_CODE,
  e.STATION_ID,
  e.METRIC_CATEGORY,
  e.AUDIENCE,           -- VARCHAR in this view (keep raw)
  e.QUANTITY,           -- spins
  e.REPORT_DATE,
  e.MODIFIED_AT
FROM LUMINATE_DB_LISTING_DETAIL.EXTRACT_S.VW_DAILY_FACT_SONG_AIRPLAY_DETAIL_DS AS e
CROSS JOIN bounds b
WHERE e.REPORT_DATE BETWEEN b.start_date AND b.end_date;
-- No ORDER BY to keep it fast; sort downstream if needed.
